...
ElfImage::ElfImage(std::string_view lib_name) : path_(lib_name) {
    ...
    if (fd < 0) {
        LOG_INFO("Failed to open ELF file: %s", path_.c_str());
        return;
    }

    struct stat file_info;
    if (fstat(fd, &file_info) < 0) {
        LOG_INFO("fstat failed for %s", path_.c_str());
        close(fd);
        return;
    }
    ...
    if (file_map_ == MAP_FAILED) {
        LOG_INFO("mmap failed for %s", path_.c_str());
        file_map_ = nullptr;
        return;
    }
...
//    LOG_INFO("Found .gnu_debugdata section in %s (%d bytes). Decompressing...", path_.c_str(),
//         debugdata_size);
    ...
    while (true) {
        ...
        if (ret == XZ_STREAM_END) {
            ...
            xz_dec_end(dec);
//            LOG_INFO("Successfully decompressed .gnu_debugdata (%d bytes)", elf_debugdata_.size());
            return true;
        }
        if (ret != XZ_OK) {
            LOG_INFO("XZ decompression failed with code %d", (int)ret);
            xz_dec_end(dec);
            return false;
        }
...
bool ElfImage::findModuleBase() {
    ...
    if (!maps) {
        LOG_INFO("Failed to open /proc/self/maps");
        return false;
    }
    ...
    while (fgets(line_buffer, sizeof(line_buffer), maps)) {
        if (strstr(line_buffer, path_.c_str())) {
            ...
            if (items_parsed >= 2) {
                ...
                LOG_INFO("Found module entry: %s", line_buffer);
            }
        }
    }
    fclose(maps);
    ...
    if (filtered_list.empty()) {
        LOG_INFO("Could not find any mappings for %s", path_.c_str());
        return false;
    }
    ...
    LOG_INFO("Found base for %s at %lx", path_.c_str(), found_block->start_addr);
    return true;
}
