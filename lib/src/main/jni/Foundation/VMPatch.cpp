...
namespace FunctionDef {
    ...
    typedef jint (*Function_cameraNativeSetupFunc_T5)(JNIEnv *, jobject, jobject, jint, jstring,
                                                      jboolean, jboolean);
...
static struct {
    ...
    union {
        ...
        Function_cameraNativeSetupFunc_T5 t5;
...
jint getCallingUid(JNIEnv *env, jclass jclazz) {
    env = Environment::ensureCurrentThreadIsAttached();
    jint uid;
    if (patchEnv.is_art) {
        uid = patchEnv.jni_orig_getCallingUid(env, jclazz);
    } else {
        uid = patchEnv.native_getCallingUid(patchEnv.IPCThreadState_self());
    }
    return env->CallStaticIntMethod(nativeEngineClass.get(), patchEnv.method_onGetCallingUid, uid);
}
        ...
        switch (patchEnv.cameraMethodType) {
            ...
            case 4:
                ...
                *jniFuncPtr = (void *) new_native_cameraNativeSetupFunc_T4;
                break;
            case 5:
                patchEnv.orig_native_cameraNativeSetupFunc.t5 = (Function_cameraNativeSetupFunc_T5) (*jniFuncPtr);
                *jniFuncPtr = (void *) new_native_cameraNativeSetupFunc_T5;
                break;
            default:
                log_err("Camera.native_setup could not be patched, because it's not of a supported type");
        }
    }
...
void hookAndroidVM(JArrayClass<jobject> javaMethods,
    ...
    initializeCachedReferences(env);
}
