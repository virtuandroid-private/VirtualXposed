...
import com.lody.virtual.client.stub.GrantPermissionsActivity;
...
class MethodProxies {
    ...
    static class HandleApplicationWtf extends MethodProxy {
        @Override
        public String getMethodName() {
        ...
        @Override
        public Object call(Object who, Method method, Object... args) throws Throwable {
            // "true if the process should exit immediately (WTF is fatal)"
            return true;
        }
    }
    ...
    static class StartActivity extends MethodProxy {
        ...
        private static final String ACTION_REQUEST_PERMISSIONS =
            mirror.android.content.pm.PackageManager.ACTION_REQUEST_PERMISSIONS.get();
        ...
        public Object call(Object who, Method method, Object... args) throws Throwable {
            ...
            if (Intent.ACTION_INSTALL_PACKAGE.equals(intent.getAction())
                ...
                handlePermissionRequest(intent, resultTo);
            }
    ...
    static class CheckPermission extends MethodProxy {
        ...
        private static final int ARG_PERMISSION = 0;
        private static final int ARG_UID = 2;
    ...
    static class GetContentProvider extends MethodProxy {
        ...
        /**
         * Resembles com.android.server.am.ContentProviderHelper.checkContentProviderPermission()
         * From call stack:
         * - android.app.IActivityManager.getContentProvider()
         * - com.android.server.am.ActivityManagerService.getContentProvider()
         * - com.android.server.am.ContentProviderHelper.getContentProvider()
         * - com.android.server.am.ContentProviderHelper.getContentProviderImpl()
         * - com.android.server.am.ContentProviderHelper.checkAssociationAndPermissionLocked()
         * - com.android.server.am.ContentProviderHelper.checkContentProviderPermission()
         */
        private String checkContentProviderPermission(ProviderInfo cpi, int callingPid, int callingUid) {
            ...
            String readPermission = cpi.readPermission;
            String writePermission = cpi.writePermission;
            if (readPermission == null && writePermission == null) {
                return null;
            }

            // Fix for MANAGE_DOCUMENTS permission. This permission cannot be granted to third party apps!
            // Redirect to the correct MANAGE_EXTERNAL_STORAGE permission.
            if(readPermission!= null && readPermission.equals(Manifest.permission.MANAGE_DOCUMENTS) ||
                    writePermission != null && writePermission.equals(Manifest.permission.MANAGE_DOCUMENTS)) {
                readPermission = Manifest.permission.MANAGE_EXTERNAL_STORAGE;
                writePermission = Manifest.permission.MANAGE_EXTERNAL_STORAGE;
            }

            if (permissionManager.checkPermission(readPermission, callingUid)
                    == PackageManager.PERMISSION_GRANTED) {
                return null;
            }
            if (permissionManager.checkPermission(writePermission, callingUid)
                    == PackageManager.PERMISSION_GRANTED) {
                return null;
            }
            return "Permission Denial: opening provider " + cpi.name
                    + " from (pid=" + callingPid + ", uid=" + callingUid + ")"
                    + " requires " + readPermission + " or " + writePermission;
        }
