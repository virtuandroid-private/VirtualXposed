...
public class VPackageManager {
    ...
    private static final String TAG = VPackageManager.class.getSimpleName();
    ...
    private void addSharedLibraries(ApplicationInfo info, List<String> libraries) {
        ...
        for (String lib : libraries) {
            String path = String.format("/system/framework/%s.jar", lib);
            if (!new File(path).exists()) {
                VLog.w(TAG, "Shared library not found: '%s'. Skipping", path);
                continue;
            }
            addSharedLibrary(info, path);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                SharedLibraryInfo sharedLibraryInfo = libNameToSharedLibraryInfo(path, lib);
    ...
    private void addSharedLibrary(ApplicationInfo info, String path) {
        String[] newSharedLibraryFiles;
        if (info.sharedLibraryFiles == null) {
            newSharedLibraryFiles = new String[]{path};
        } else {
    ...
    public static SharedLibraryInfo libNameToSharedLibraryInfo(String path, String name) {
        return mirror.android.content.pm.SharedLibraryInfo.createSharedLibInfo(
                path, null, null,
                name, -1, 0, new VersionedPackage("android", 0), null,
                null, false);
    }
    ...
    public boolean canRequestPackageInstalls() {
         try {
            return getInterface().canRequestPackageInstalls();
        } catch (RemoteException e) {
            return VirtualRuntime.crash(e);
        }
    }
