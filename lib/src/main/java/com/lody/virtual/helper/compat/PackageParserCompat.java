...
import mirror.android.content.pm.pkg.FrameworkPackageUserStateDefault;
...
public class PackageParserCompat {
    ...
    static {
        // Initialize sUserState
        if (API_LEVEL >= TIRAMISU) {
            sUserState = FrameworkPackageUserStateDefault.ctor.newInstance();
        } else if (API_LEVEL >= JELLY_BEAN_MR1) {
            sUserState = PackageUserState.ctor.newInstance();
        } else {
            sUserState = null;
        }
    }
    ...
    public static PackageInfo generatePackageInfo(Package p, int flags, long firstInstallTime, long lastUpdateTime) {
        if (API_LEVEL >= TIRAMISU) {
            return PackageParserTiramisu.generatePackageInfo.call(
                p,
                GIDS,
                flags,
                firstInstallTime,
    ...
    public static void collectCertificates(PackageParser parser, Package p, int flags) throws Throwable {
        if (API_LEVEL >= TIRAMISU) {
            PackageParserTiramisu.collectCertificates.callWithException(p, true);
