...
public class VAppManagerService extends IAppManager.Stub {
    ...
    private final VPermissionManager mPermissionManager = VPermissionManager.get();
    ...
    public synchronized InstallResult installPackage(String path, int flags, boolean notify) {
        ...
        if (!packageFile.exists()) {
            return InstallResult.makeFailure("Package File does not exist.");
        }
        VPackage pkg = null;
        try {
        ...
        ps.appId = VUserHandle.getAppId(mUidSystem.getOrCreateUid(0, pkg));
        if (res.isUpdate) {
        ...
        try {
            ...
            e.printStackTrace();
            return InstallResult.makeFailure("Failed to set permissions");
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            for (File file : appDir.listFiles()) {
                // Fix 'writable dex file is not allowed' error on Android 13+
                file.setReadOnly();
            }
        }
        BroadcastSystem.get().startApp(pkg);
    ...
    public synchronized boolean installPackageAsUser(int userId, String packageName) {
        ...
        if (!VUserManagerService.get().exists(userId) || ps == null || ps.isInstalled(userId)) {
            return false;
        }
        ps.setInstalled(userId, true);
        File packageFile = new File(ps.apkPath);
        if (!packageFile.exists()) {
            return false;
        }
        VPackage pkg = null;
    ...
    public synchronized boolean uninstallPackage(String packageName) {
        ...
        if (ps != null) {
            ...
            mPermissionManager.removePermissionsForUid(ps.appId);
            return true;
        }
        return false;
    }
    ...
    public synchronized boolean uninstallPackageAsUser(String packageName, int userId) {
        ...
        if (ps != null) {
            ...
            final int uid = VPackageManager.get().getPackageUid(packageName, userId);
            mPermissionManager.removePermissionsForUid(uid);
            return true;
        }
        return false;
    }
