...
void (*suspendAll)(ScopedSuspendAll*, const char*) = nullptr;
...
void init_entries(JNIEnv *env) {
    ...
    if (api_level < 23) {
        ...
        suspendAll = reinterpret_cast<void (*)(ScopedSuspendAll*, const char*)>(dlsym_ex(handle, "_ZN3art16ScopedSuspendAllC1EPKcb"));
...
jboolean epic_munprotect(JNIEnv *env, jclass, jlong addr, jlong len) {
    ...
    LOGV("munprotect page size: %ld, alignment: %d", pagesize, alignment);
...
jboolean epic_cacheflush(JNIEnv *env, jclass, jlong addr, jlong len) {
    ...
    LOGV("arm cacheflush for, %lld", addr);
    if (i == -1) {
...
jboolean epic_activate(JNIEnv* env, jclass jclazz, jlong jumpToAddress, jlong pc, jlong sizeOfDirectJump,
    ...
    if (result) {
        ...
        long address = jumpToAddress;
        LOGV("Writing hook failed: Unable to unprotect memory at %ld", address);
    }
